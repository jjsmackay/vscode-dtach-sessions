...
Showing each session's live Claude run-state in the panel: the state vocabulary (working / tool / waiting / done / idle), sourcing from per-hash status files joined to sessions by hash, staleness decay of transient states, and refresh on status change. Composes with attach-state without either suppressing the other, and de-emphasises detached rows so attach-state stays legible even when a run-state owns the row icon.
