# session-rename Specification

## Purpose

Renaming a dtach session in place: moving its socket file to a new name while preserving the session's stable hash (so it stays live and killable) and keeping any open terminal's tab title consistent.
